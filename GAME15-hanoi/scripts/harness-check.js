...
function collectJsFiles(pathOrFile) {
  try {
    ...
    return readdirSync(pathOrFile, { withFileTypes: true }).flatMap((e) =>
      e.isDirectory() ? collectJsFiles(join(pathOrFile, e.name))
      : extname(e.name) === '.js' ? [join(pathOrFile, e.name)] : []
    );
  } catch { return []; }
}
...
for (const file of jsFiles) {
  ...
  for (const { pattern, label } of FORBIDDEN) {
    lines.forEach((line, i) => {
      if (pattern.test(line)) { console.error(`FORBIDDEN [${label}] ${file}:${i+1}  ${line.trim()}`); forbidden++; }
    });
  }
}
